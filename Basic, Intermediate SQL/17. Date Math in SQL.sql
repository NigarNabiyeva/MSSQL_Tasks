/*
1. Use date math in conjunction with GETDATE to calculate what date is 100 days from now.
*/
...
/*
2. Use DATEADD to calculate what date is 6 months from now.
*/
...
/*
3. Use DATEDIFF to select all rows from the "Sales.SalesOrderHeader" table 
...
WHERE DATEDIFF(DAY, [OrderDate], DATEFROMPARTS(2013,12,25)) <= 7;


/*
4. Modify your query from Exercise 3 to work for ANY year, not just 2013. 
...
SELECT
	*
FROM [AdventureWorks2019].[Sales].[SalesOrderHeader]
WHERE DATEDIFF(DAY, [OrderDate], DATEFROMPARTS(YEAR([OrderDate]), 12,25)) <= 7;